...
from constants.configs import (
    ...
    SPAM_WINDOW_SECONDS, 
    MAX_WARNINGS,
    AUTOMOD_CLEANUP_INTERVAL_SECONDS, 
...
class AutoMod(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        """
        self.bot = bot
        self.user_messages: Dict[int, deque] = defaultdict(lambda: deque(maxlen=SPAM_TRACK_MESSAGE_COUNT))
        self.recently_warned: Dict[Tuple[int, int], float] = {}
        self._bg_tasks = set()
        self.logger = logging.getLogger("bot")
        self._cleanup_interval = AUTOMOD_CLEANUP_INTERVAL_SECONDS 
        self._message_age = AUTOMOD_MESSAGE_AGE_SECONDS
    ...
    async def warn_user(self, message):
        ...
        await message.channel.send(f"⚠️ {user.mention}, please stop spamming! Warning `{count}`/{MAX_WARNINGS}.")
